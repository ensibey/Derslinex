...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#FAF8F5]/95 backdrop-blur-md shadow-xs border-b border-[#EFECE6]">
        ...
        <div className="flex items-center justify-between h-16 gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            ...
            <Link href="/" className="flex items-center gap-2 group flex-shrink-0">
              <div className="relative w-10 h-10 sm:w-11 sm:h-11 group-hover:scale-105 transition-transform flex items-center justify-center">
                <Image
                  src="/logo.png?v=9"
                  alt="Derslinex Logo"
                  fill
              ...
              <span className="text-xl sm:text-2xl font-black text-[#1E3A8A] tracking-tight group-hover:text-[#B45309] transition-colors whitespace-nowrap">Derslinex</span>
            </Link>
          </div>
          ...
          <nav className="hidden xl:flex items-center gap-4 2xl:gap-6 flex-shrink">
            {navLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="text-xs 2xl:text-sm font-bold text-[#1E3A8A]/80 hover:text-[#B45309] transition-colors whitespace-nowrap"
              >
                {l.label}
              </Link>
            ))}
          ...
          {/* CTA & Profile Avatar (Desktop/Laptop lg screen: 1024px+) */}
          <div className="hidden lg:flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            >
              ...
              <span>WhatsApp ile Ders Al</span>
            </a>
            ...
            {user ? (
              <Link
                href="/profil"
                className="flex items-center gap-2 bg-white border border-[#1E3A8A]/20 hover:border-[#1E3A8A] px-3 py-1.5 rounded-xl transition-all shadow-xs hover:shadow-md group whitespace-nowrap"
                aria-label="Profil Sayfası"
              >
                <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#1E3A8A] to-indigo-800 flex items-center justify-center text-white font-black overflow-hidden flex-shrink-0">
                  {user?.avatar ? (
                ...
                <div className="flex flex-col text-left">
                  <span className="text-xs font-black text-[#1E3A8A] truncate max-w-[90px] leading-tight">
                    {user.name.split(" ")[0]}
                  </span>
