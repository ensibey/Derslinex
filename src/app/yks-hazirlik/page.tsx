      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {[
              ...
              <button
                ...
                className={`flex items-start text-left gap-4 border rounded-3xl p-6 transition-all duration-300 ${activeTool === t.id ? "bg-primary-600 text-white border-primary-600 shadow-md -translate-y-1" : "bg-white text-gray-700 border-gray-100 shadow-sm"}`}
              >
                <span className="text-3xl">{t.emoji}</span>
                <div>
                  <p className={`font-bold text-base sm:text-lg ${activeTool === t.id ? "text-white" : "text-gray-900"}`}>{t.title}</p>
        ...
        <section className="mb-16 bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
            ...
            <div className="text-center py-6">
              ...
              <div className="flex justify-center gap-4 sm:gap-8">
                {[
                  ...
                  <div key={item.label} className="bg-primary-50/50 border border-primary-100 rounded-2xl p-4 sm:p-6 w-24 sm:w-32 flex flex-col items-center">
                    <span className="text-3xl sm:text-5xl font-black text-primary-600">{item.value}</span>
                    <span className="text-xs sm:text-sm text-gray-500 font-bold mt-2">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTool === "calculator" && (
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-6 text-center">🧮 TYT Tahmini Puan Hesaplama</h3>
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="bg-primary-50/50 border border-primary-100 rounded-3xl p-6 flex flex-col justify-center items-center text-center">
        ...
        <section className="mb-16">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              ...
              { tur: "AYT Sayısal", renk: "from-violet-600 via-violet-700 to-primary-850", aciklama: "180 dakika, 80 soru. Matematik, Fizik, Kimya ve Biyoloji soruları.", dersler: "Matematik, Fizik, Kimya, Biyoloji" },
              { tur: "AYT Sözel", renk: "from-rose-600 to-rose-700", aciklama: "180 dakika, 80 soru. Edebiyat, Tarih, Coğrafya, Felsefe grup soruları.", dersler: "Edebiyat, Tarih 1-2, Coğrafya 1-2, Felsefe" },
              { tur: "AYT Eşit Ağırlık", renk: "from-amber-600 to-amber-700", aciklama: "Matematik ve Türkçe-Sosyal derslerinin birleşimi. EA bölümleri için.", dersler: "Matematik, Edebiyat, Tarih 1, Coğrafya 1" },
              { tur: "AYT Dil", renk: "from-emerald-600 to-emerald-700", aciklama: "80 soru. Yabancı dil sınavı. İngilizce, Almanca, Fransızca seçenekleri.", dersler: "İngilizce / YDT" },
            ].map((s) => (
              <div key={s.tur} className={`bg-gradient-to-br ${s.renk} rounded-3xl p-8 text-white shadow-md hover:shadow-xl transition-all duration-300`}>
                <h3 className="text-xl font-black mb-3">{s.tur}</h3>
                <p className="text-white/90 text-xs mb-6 leading-relaxed">{s.aciklama}</p>
        ...
        <section className="mb-16">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/dersler" className="text-primary-600 font-bold hover:underline text-sm">Tümü →</Link>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {dersAlanlari.slice(0, 8).map((d) => (
              <Link key={d.id} href={`/dersler/${d.slug}`}
                className="flex items-center gap-2 bg-white border border-gray-100 hover:border-primary-300 hover:shadow-sm px-4 py-3 rounded-xl text-sm font-semibold transition-all text-gray-700">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/blog" className="text-primary-600 font-bold hover:underline text-sm">Tüm Yazılar →</Link>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
